...
import type { LogCardEntry } from "@/app/(public)/field-logs/components/LogCard";

type FeaturedLogProps = {
	log: LogCardEntry;
};

export function FeaturedLogsSection({ log }: FeaturedLogProps) {
	return (
			...
			<div className="mx-auto max-w-7xl">
				<div className="relative">
					<div className="mb-6 flex items-center gap-2">
						...
						<span className="text-xs uppercase tracking-wider text-amber-400 font-mono-ui">
							...
							<div className="relative aspect-16/10 overflow-hidden lg:col-span-5 lg:aspect-auto">
								...
								<div className="absolute top-6 left-6">
									<div className="rounded-lg border border-amber-500/30 bg-black/60 px-4 py-2 backdrop-blur-sm">
										<span className="text-sm text-amber-400 font-mono-ui">{log.id}</span>
									</div>
								</div>

								<div className="absolute right-6 bottom-6 left-6">
									<div className="flex items-center gap-4 text-xs text-gray-400 font-mono-ui">
								...
								<div className="mb-8">
									...
									<div className="flex flex-wrap gap-2">
										{log.tags.map((tag) => (
											<span
												key={tag}
												className="font-mono-ui rounded-full border border-white/10 bg-white/5 px-3 py-1.5 text-sm text-gray-300"
											>
												{tag}
											</span>
										))}
